...
const Header = () => {
    ...
    return (
        <header className="h-20 bg-black flex px-10">
            <nav className="w-full mx-auto flex justify-between items-center px-2 text-white font-mono font-semibold text-xl">
                <Link href={HOME_ROUTE}><div>ACTIVITY 7 ADV102</div></Link>
                <ul className="flex gap-4">
                    {!user?.isLogin &&
                        <>
                            <Link href={LOGIN_ROUTE}><li className="hover:bg-gray-400 p-5 rounded-md">LOGIN</li></Link>
                            <Link href={REGISTER_ROUTE}><li className="hover:bg-gray-400 p-5 rounded-md">REGISTER</li></Link>
                        </>
                    }
                    {user?.isLogin &&
                        <>
